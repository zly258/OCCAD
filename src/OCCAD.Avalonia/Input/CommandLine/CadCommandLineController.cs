...
internal sealed class CadCommandLineController : IDisposable
{
    ...
    private void RefreshToolPrompt()
    {
        ...
        if (_workspace.Precision.PointMode == CadPointInputMode.Offset)
            prompt = _workspace.Precision.AwaitingOffsetOrigin
                ? CadLanguageManager.Text("Cad.Command.OffsetOrigin", "Pick offset base point, then enter the relative displacement.")
                : CadLanguageManager.Text("Cad.Command.OffsetDisplacement", "Enter displacement from the offset base point (# uses absolute coordinates).");
