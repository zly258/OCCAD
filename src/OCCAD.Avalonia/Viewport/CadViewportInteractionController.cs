...
internal sealed class CadViewportInteractionController : IDisposable
{
    ...
    private readonly IDisposable _selectionWindowChannel;
    private readonly IDisposable _selectionMarkerChannel;
        ...
        Dispatcher dispatcher)
    {
        ...
        _selectionWindowChannel = workspace.Transients.Register(CadTransientChannel.SelectionWindow,
            CancelSelectionGesture, () => _selectionRectangleVisible || _selectionGesture);
        _selectionMarkerChannel = workspace.Transients.Register(CadTransientChannel.SelectionMarkers,
            ClearSubobjectMarkers, () => _subobjectMarkers.Count > 0, CadTransientLifetime.Workspace);
    ...
    public void Dispose()
    {
        if (_disposed) return;
        _disposed = true;
        _selectionWindowChannel.Dispose();
        _selectionMarkerChannel.Dispose();
        ...
        OcctAvaloniaSelectionEventArgs input)
    {
        ...
        if (_workspace.Selection.Scope == CadSelectionScope.Entity &&
            (tool is null || tool.InteractionPolicy.SelectionEnabled))
        {
    ...
    private void CompleteSelectionGesture()
    {
        ...
        if (rectangleVisible)
        {
            ...
            {
                engine.SelectRectangle(startX, startY, endX, endY, false, allowOverlap);
                var hits = engine.GetSelectedHits().ToArray();
                engine.ClearSelection();
                var operation = SelectionOperation(modifiers);
                if (operation == CadSelectionOperation.Replace) _workspace.Subobjects.Clear();
                foreach (var hit in hits.Distinct())
                {
                    var parent = _workspace.Document.FindByViewerObject(hit.Owner);
                    if (parent is null || !hit.IsSubshape) continue;
                    var reference = CadSubshapeReference.Create(parent, hit.SubshapeType, hit.SubshapeIndex);
                    CadSubshapeReferenceResolver.TryGetRepresentativePoint(engine, parent, reference, out var point);
                    _workspace.Subobjects.Apply(new CadSubobjectSelection(parent, hit.SubshapeType, hit.SubshapeIndex, point),
                        operation == CadSelectionOperation.Replace ? CadSelectionOperation.Add : operation);
                }
                return;
            }
        ...
        if (_workspace.Selection.Scope == CadSelectionScope.Subobject)
        {
            if (_workspace.Preselection.Current is { IsSubshape: true } hit)
                _workspace.Subobjects.Select(hit, SelectionOperation(modifiers));
            else if (SelectionOperation(modifiers) == CadSelectionOperation.Replace)
                _workspace.Subobjects.Clear();
            return;
        }
    ...
    private void CancelSelectionGesture()
    {
        ...
        _selectionGesture = false;
        if (_workspace.Engine is not { IsInitialized: true }) _selectionRectangleVisible = false;
    }
