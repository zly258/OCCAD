...
internal sealed class CadPropertyInspectorController : IDisposable
{
    ...
    private Control CreateEditor(PropertySlot slot)
    {
        ...
        return CadPropertyEditorFactory.Resolve(descriptor, entityContext) switch
        {
            CadPropertyEditorKind.Layer =>
                CreateLayerEditor(descriptor, value as string, isMixed),
    ...
    private Control CreateLayerEditor(CadPropertyDescriptor descriptor, string? value, bool isMixed)
    {
        var combo = new ComboBox
        {
            HorizontalAlignment = HorizontalAlignment.Stretch,
            VerticalAlignment = VerticalAlignment.Center,
            ItemsSource = descriptor.GetChoices(_workspace),
    ...
    private void ApplyLayerValue(PropertySlot slot, object? value)
    {
        ...
        try
        {
            using var transaction = _workspace.BeginTransaction($"Property {slot.Descriptor.Name}");
