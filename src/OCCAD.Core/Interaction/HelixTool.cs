...
public sealed class HelixTool : CadDrawingTool, ICadPointInputTool
{
    ...
    private void Update(OcctPoint3d p) { var d = p - _origin; d = d - _axis * d.Dot(_axis); if (!d.TryNormalize(out var x)) return; Context.Preview.Show(new CadHelixEntity(_origin, _axis, x, d.Length, d.Length, _turns)); }
    private void Show() => Context.Preview.Show(new CadHelixEntity(_origin, _axis, _xAxis, _radius, _pitch, _turns));
}
