...
internal sealed class CadFloatingToolPanel : Border, IDisposable
{
    ...
    private void CommitExactPoint(CadTool tool, TextBox editor)
    {
        ...
        if (!_workspace.Tools.SubmitPointText(editor.Text))
        {
            ShowFeedback("Cad.Text.PointRejected", "The current tool cannot accept that point.");
            editor.SelectAll();
            return;
        }
    ...
    private void AddCommandButtons(CadTool tool)
    {
        ...
        _back.IsEnabled = tool.CanStepBack || _workspace.Precision.PointMode == CadPointInputMode.Offset;
