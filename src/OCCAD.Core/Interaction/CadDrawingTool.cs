...
public abstract class CadDrawingTool : CadTool
{
    ...
    protected void CommitPreview(CadEntity? entity)
    {
        ...
        committed.Layer = Context.Workspace.Layers.Current.Name;
        var engine = Context.Engine;

        // Preview and persistent geometry must never coexist across the commit
        // boundary. The lightweight create transaction installs the Undo entry
        // only after tool deactivation and common transient cleanup succeed.
        ...
        try
        {
            CadTransaction.ApplyCreatedEntity(
                Context.Workspace,
                committed,
                $"Create {committed.Name}",
                Context.Workspace.Tools.CompleteCurrent);
        }
        catch
        {
            // If applying the persistent entity failed before the tool left the
            // drawing state, restore the user's last valid preview so the command
            // can continue. A tool-deactivation failure leaves the tool neutral;
            // ApplyCreatedEntity rolls the document change back instead.
            if (IsActive)
                ShowPreview(entity);
            throw;
        }
